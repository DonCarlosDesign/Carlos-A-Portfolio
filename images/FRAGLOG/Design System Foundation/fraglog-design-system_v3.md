...
## Typography
- One family only: Cascadia Mono, loaded as a proper embedded web font (e.g. via @fontsource/cascadia-mono on jsDelivr) so it renders identically for every visitor regardless of what's installed locally — never referenced by name alone. No separate display/pixel typeface. A second "pixel/LCD" font was tried and rejected for reading too much like a retro arcade game rather than a serious terminal tool; IBM Plex Mono was also tried and rejected for not matching the Consolas-like feel of the original concept — Cascadia Mono is Microsoft's own modern successor to Consolas, same design lineage, freely embeddable.
- Hero numbers (rank, win rate, K/D, streak counters): same Cascadia Mono family, semibold/600, at a large size — the weight and scale alone create the "hero" moment, no separate face needed
- Labels are lowercase with letter-spacing (~1px) — reinforces the "terminal readout" feel over standard UI casing
- Two weights only: regular (400) for data/body, semibold (600) for hero numbers and win states. Never use Light weight.
- Sentence/lowercase casing applies to hero *values* too, not just labels — e.g. the rank value renders as `radiant`, not `RADIANT`. No text anywhere in the system should be all-caps; this was an early inconsistency that got caught and fixed.
